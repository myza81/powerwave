...
def test_repeated_harmonic_panel_build_reuses_extraction_cache(qapp, monkeypatch) -> None:
    from app.analytics.harmonics import harmonic_extraction
    from app.ui.main_window.main_window import PowerwaveMainWindow
    from app.visualization.widgets.flexible_plot_canvas import FlexiblePlotCanvas

    record = _single_channel_record("VA", "V")
    calls = {"count": 0}
    original_extract = harmonic_extraction.extract_harmonics

    def counting_extract(*args, **kwargs):
        calls["count"] += 1
        return original_extract(*args, **kwargs)

    monkeypatch.setattr(harmonic_extraction, "extract_harmonics", counting_extract)
    ...
    try:
        first = win._build_harmonic_panels(record, None, _factory)
        count_after_first = calls["count"]
        second = win._build_harmonic_panels(record, None, _factory)

        assert "thd_voltage" in first
        assert "harmonic_spectrum_voltage" in second
...
def test_spectrum_panel_cursor_updates_do_not_create_plot_items(qapp) -> None:
    from app.ui.main_window.main_window import PowerwaveMainWindow
    from app.visualization.widgets.flexible_plot_canvas import FlexiblePlotCanvas
    ...
    try:
        panels = win._build_harmonic_panels(
            _single_channel_record("VA", "V"), None, _factory
        )
        spectrum = panels["harmonic_spectrum_voltage"]
        before = len(spectrum._primary_plot.items)
        spectrum.setVisible(False)
        for pos in (0.02, 0.04, 0.08, 0.12):
            spectrum.set_cursor_pos(pos)
        after = len(spectrum._primary_plot.items)
        assert after == before
    finally:
        for c in canvases_created:
            c.close()
